...
describe("RbacPostgresAssignmentAdapter", function () {
  ...
  it("should store many and load them", async () => {
    const adapter: RbacAssignmentAdapter = new RbacPostgresAssignmentAdapter({
      client,
    });
...
describe("RbacPostgresItemChildAdapter", function () {
  ...
  it("should store many and load them", async () => {
    const adapter: RbacItemChildAdapter = new RbacPostgresItemChildAdapter({
      client,
    });
    const values = [
      $.admin_manager,
  ...
  it("should find many by parent", async () => {
    ...
    expect(entry).to.have.deep.members([
      $.admin_manager,
      $.admin_updateProfile,
    ]);
  });
