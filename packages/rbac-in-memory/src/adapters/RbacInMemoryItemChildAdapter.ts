...
class RbacInMemoryItemChildAdapter implements RbacItemChildAdapter {
  ...
  async find(parent: RbacItem["name"], child: RbacItem["name"]) {
    return (
      this.entries.find((x) => x.parent === parent && x.child === child) ?? null
    );
  }
...
export default RbacInMemoryItemChildAdapter;
